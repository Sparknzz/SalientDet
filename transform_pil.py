...
class Compose(object):
    def __init__(self, transforms, metadata_key):
        self.transforms = transforms
        self.metadata_key = metadata_key

    def __call__(self, image, target, metadata):
...
class Resize(object):
    def __init__(self, size, keep_aspect_ratio=False):
        self.size = size
        self.keep_aspect_ratio = keep_aspect_ratio

    def __call__(self, image, target):
        if self.keep_aspect_ratio:
            image = pad_if_smaller(image, max(image.size))
            target = pad_if_smaller(target, max(image.size))
